...
- [x] `UsersModule` criado e importado no `AppModule`
- [x] `UsersService` com os métodos:
...
- [x] `CreateUserDto` com validações (`class-validator`):
  - `email` — obrigatório, formato email
  - `password` — obrigatório, mínimo 8 caracteres
  - `role` — opcional, default `admin`
- [x] Senha nunca retornada em nenhum método público
- [x] **Seed script** (`prisma/seed.ts`) criado:
  - Cria um usuário admin padrão usando variáveis de ambiente
  - Executa com `npx prisma db seed`
- [x] `package.json` com script `prisma.seed` apontando para o seed
