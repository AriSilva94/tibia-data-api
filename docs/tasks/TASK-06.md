...
- [ ] `AuthModule` criado e registrado no `AppModule`
...
- [ ] **Endpoint `GET /api/v1/auth/me`** (protegido):
  - Retorna dados do usuário logado
  - Retorna `401` se token inválido ou ausente
- [ ] `JwtStrategy` implementada com `passport-jwt`:
...
- [ ] `JWT_SECRET` lido via `ConfigService`
- [ ] `JWT_EXPIRES_IN` configurável (ex: `7d`)
